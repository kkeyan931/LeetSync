...
public:
    bool backtrack(vector<int>& arr, int index, int count, int currSum, int k,
        ...
        if (currSum == targetSum) {
            return memo[taken] = backtrack(arr, 0, count + 1, 0, k, targetSum,
                                           taken, memo);
        }
        ...
        for (int j = index; j < n; ++j) {
            if (taken[j] == '0') {
                ...
                // If using current jth element in this subset leads to make all
                // valid subsets.
                if (backtrack(arr, j + 1, count, currSum + arr[j], k, targetSum,
                              taken, memo)) {
                    return true;
                }
        ...
        // We were not able to make a valid combination after picking each
        // element from the array, hence we can't make k subsets.
